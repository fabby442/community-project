...
<style>
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  #app { display:flex; height:100vh; opacity:0; transition:opacity .4s; pointer-events:none; }
  ...
  .feed-center { flex:1; max-width:600px; padding:24px; overflow-y:auto; }
  ...
  .stories-row { display:flex; gap:12px; margin-bottom:24px; overflow-x:auto; padding-bottom:4px; }
  ...
  .story-ring { width:62px; height:62px; border-radius:50%; padding:2.5px; background:var(--gradient); transition:transform .2s; }
  ...
  .story-add-btn {
    ...
    display:flex; align-items:center; justify-content:center;
    font-size:24px; color:var(--accent); cursor:pointer; flex-shrink:0; transition:all .2s;
  }
  ...
  .create-post { background:var(--surface); border:1px solid var(--border); border-radius:16px; padding:16px; margin-bottom:20px; }
  ...
  .icon-btn {
    ...
    display:flex; align-items:center; gap:6px; transition:all .2s; font-family:'DM Sans',sans-serif;
  }
  .icon-btn:hover { border-color:var(--accent); color:var(--accent); }
  .post-btn { margin-left:auto; padding:8px 20px; font-size:13px; font-weight:600; }

  .post-card { background:var(--surface); border:1px solid var(--border); border-radius:16px; padding:18px; margin-bottom:16px; transition:border-color .2s; }
  ...
  .suggestions-box { background:var(--surface); border:1px solid var(--border); border-radius:16px; padding:18px; margin-bottom:16px; }
  ...
  .convo-list { width:280px; border-right:1px solid var(--border); display:flex; flex-direction:column; overflow:hidden; }
  ...
  .msg-bubble .bubble-inner { padding:10px 14px; border-radius:16px; font-size:14px; line-height:1.5; background:var(--surface2); color:var(--text); }
  .msg-bubble.mine .bubble-inner { background:linear-gradient(135deg, #7c3aed, #c084fc); color:#fff; }
  .msg-bubble .bubble-time { font-size:10px; color:var(--muted); margin-top:3px; text-align:right; }
  .chat-input-area { padding:16px 20px; border-top:1px solid var(--border); display:flex; gap:10px; align-items:center; }
  .chat-input {
  ...
  .explore-tile {
    ...
    display:flex; align-items:center; justify-content:center;
    cursor:pointer; transition:opacity .2s; font-size:32px;
  }
  ...
  .profile-info .pname { font-size:24px; font-weight:700; font-family:'DM Serif Display',serif; }
  ...
  .story-modal { display:none; position:fixed; inset:0; background:rgba(0,0,0,.9); z-index:500; align-items:center; justify-content:center; }
  .story-modal.open { display:flex; }
  .story-view { width:380px; height:620px; max-height:90vh; border-radius:20px; overflow:hidden; position:relative; background:var(--gradient); display:flex; flex-direction:column; }
  ...
  .story-prog-fill { height:100%; background:#fff; transition:width 5s linear; width:0%; }
  ...
  .story-reply-input {
    ...
    border-radius:24px; padding:9px 16px; color:#fff; font-family:'DM Sans',sans-serif; font-size:13px; outline:none;
  }
...
<div id="auth-screen">
  <div class="auth-box">
    ...
    <div class="auth-form" id="login-form">
      ...
      <div style="color:var(--muted);font-size:12px;text-align:center;">Try: lunapark / luna123</div>
    </div>
    <div class="auth-form hidden" id="signup-form">
      ...
      <div class="field-group">
        <label>Username</label>
        <input type="text" id="su-user" placeholder="coolhandle" />
      </div>
      <div class="field-group">
        <label>Bio</label>
        <input type="text" id="su-bio" placeholder="Tell the world about you..." />
      </div>
      <div class="field-group">
        <label>Password</label>
...
<div id="app">
  <div class="sidebar">
    ...
    <div class="nav-item" id="nav-explore" onclick="showPage('explore')"><span class="icon">🔍</span> Explore</div>
    <div class="nav-item" id="nav-notifications" onclick="showPage('notifications')"><span class="icon">🔔</span> Notifications</div>
    <div class="nav-item" id="nav-messages" onclick="showPage('messages')"><span class="icon">💬</span> Messages</div>
    <div class="nav-item" id="nav-profile" onclick="showPage('profile')"><span class="icon">👤</span> Profile</div>
        ...
        <div class="feed-center">
          ...
          <div class="create-post">
            <div class="create-post-top">
              <div class="avatar" id="post-creator-avatar"></div>
            ...
            <div class="create-post-actions">
              ...
              <button class="btn-primary icon-btn post-btn" onclick="createPost()">Post</button>
            </div>
          </div>
...
<div class="story-modal" id="story-modal" onclick="closeStory(event)">
  <div class="story-view">
    ...
    <div class="story-reply">
      <input class="story-reply-input" id="story-reply-input" placeholder="Reply..." />
...
function renderSidebar() {
  document.getElementById('sidebar-avatar').textContent = currentUser.emoji;
  document.getElementById('sidebar-name').textContent = currentUser.name;
  document.getElementById('sidebar-handle').textContent = '@'+currentUser.username;
  document.getElementById('post-creator-avatar').textContent = currentUser.emoji;
}

function renderFeed() { renderStories(); renderPosts(); renderSuggestions(); }

function renderStories() {
  const row = document.getElementById('stories-row');
  row.innerHTML = '';
  const addWrap = document.createElement('div');
  addWrap.style.cssText = 'display:flex;flex-direction:column;align-items:center;gap:6px;';
  addWrap.innerHTML = `<div class="story-add-btn" onclick="addStory()">+</div><span class="story-label">Your story</span>`;
  row.appendChild(addWrap);
  const storyUserIds = [...new Set(DB.stories.map(s=>s.userId))];
  storyUserIds.forEach(uid => {
    ...
    div.innerHTML = `<div class="story-ring ${allSeen?'seen':''}"><div class="story-inner">${u.emoji}</div></div><span class="story-label">${uid===currentUser.id?'You':u.name.split(' ')[0]}</span>`;
    row.appendChild(div);
  });
}
...
function openStory(userId) {
  ...
  prog.innerHTML = stories.map((_,i)=>`<div class="story-prog-bar"><div class="story-prog-fill" id="prog-${i}"></div></div>`).join('');
...
function renderPosts() {
  document.getElementById('posts-feed').innerHTML = [...DB.posts].reverse().map(p => renderPostHTML(p)).join('');
}
...
function renderPostHTML(post) {
  ...
  const commentsHTML = post.comments.map(c => {
    ...
    return `<div class="comment-item"><div class="avatar sm">${cu?cu.emoji:'👤'}</div><div class="comment-content"><div class="comment-author">${cu?cu.name:'Unknown'}</div><div class="comment-text">${c.text}</div></div></div>`;
  }).join('');
  return `<div class="post-card" id="post-${post.id}">
    <div class="post-header"><div class="avatar">${u.emoji}</div><div class="post-meta"><div class="name">${u.name}</div><div class="time">@${u.username} · ${post.time} ago</div></div></div>
    <div class="post-content">${post.text}</div>
    <div class="post-actions">
      <button class="post-action-btn ${liked?'liked':''}" onclick="toggleLike('${post.id}')">${liked?'❤️':'🤍'} ${post.likes.length}</button>
      <button class="post-action-btn" onclick="toggleComments('${post.id}')">💬 ${post.comments.length}</button>
...
function createPost() {
  ...
  if (!text) return;
  DB.posts.unshift({ id:'p'+Date.now(), userId:currentUser.id, text, likes:[], comments:[], time:'just now', emoji:'✨' });
  input.value='';
  renderPosts();
...
function toggleLike(postId) {
  ...
  if (idx===-1) { post.likes.push(currentUser.id); } else { post.likes.splice(idx,1); }
  const el = document.getElementById('post-'+postId);
  if (el) { const tmp=document.createElement('div'); tmp.innerHTML=renderPostHTML(post); el.replaceWith(tmp.firstChild); }
}
...
function addComment(postId) {
  ...
  if (list) {
    ...
    div.innerHTML=`<div class="avatar sm">${currentUser.emoji}</div><div class="comment-content"><div class="comment-author">${currentUser.name}</div><div class="comment-text">${text}</div></div>`;
...
function renderSuggestions() {
  ...
  document.getElementById('suggestions-list').innerHTML = others.map(u=>`
    <div class="suggest-item">
      <div class="avatar sm">${u.emoji}</div>
      <div class="suggest-info"><div class="name">${u.name}</div><div class="bio">${u.bio.substring(0,28)}...</div></div>
      <button class="follow-btn" onclick="toggleFollow('${u.id}')">${currentUser.following.includes(u.id)?'Following':'Follow'}</button>
    </div>`).join('') || '<div style="color:var(--muted);font-size:13px;">No suggestions right now</div>';
}
...
function toggleFollow(userId) {
  ...
  if (idx===-1) { currentUser.following.push(userId); u.followers.push(currentUser.id); showToast('Following '+u.name+' ✨'); }
  else { currentUser.following.splice(idx,1); u.followers.splice(u.followers.indexOf(currentUser.id),1); showToast('Unfollowed '+u.name); }
  renderSuggestions(); renderProfile();
}

function renderExplore() { doSearch(document.getElementById('search-input').value.trim()); }

function doSearch(q) {
  const cont = document.getElementById('explore-content');
  if (!q) {
    const emojis = ['🌅','🎵','🌿','🏙️','🎨','🍜','🌊','📚','🎸','🌺','🏔️','✨'];
    cont.innerHTML = `<div class="explore-grid">${emojis.map(e=>`<div class="explore-tile">${e}</div>`).join('')}</div>`;
    return;
  }
  const results = DB.users.filter(u=>u.id!==currentUser.id&&(u.name.toLowerCase().includes(q.toLowerCase())||u.username.toLowerCase().includes(q.toLowerCase())));
  cont.innerHTML = `<div class="search-results">${results.length?results.map(u=>`
    <div class="user-result">
      <div class="avatar">${u.emoji}</div>
      <div class="info"><div class="name">${u.name}</div><div class="handle">@${u.username}</div></div>
      <button class="follow-btn ${currentUser.following.includes(u.id)?'following':''}" onclick="toggleFollow('${u.id}')">${currentUser.following.includes(u.id)?'Following':'Follow'}</button>
    </div>`).join(''):`<div style="color:var(--muted);padding:20px;">No users found for "${q}"</div>`}</div>`;
}

function renderNotifications() {
  const icons = { like:'❤️', follow:'👤', comment:'💬', post:'📝' };
  const getMsg = n => {
    const u = DB.users.find(x=>x.id===n.fromUserId);
    if (!u) return '';
    if (n.type==='like') return `<span class="bold">${u.name}</span> liked your post`;
    if (n.type==='follow') return `<span class="bold">${u.name}</span> started following you`;
    if (n.type==='comment') return `<span class="bold">${u.name}</span> commented: "${n.text||''}"`;
    if (n.type==='post') return `<span class="bold">${u.name}</span> posted something new`;
    return n.type;
  };
  document.getElementById('notifs-list').innerHTML = DB.notifications.map(n=>`
    <div class="notif-item">
      <div class="notif-icon">${icons[n.type]||'🔔'}</div>
      <div class="notif-text">${getMsg(n)}</div>
      <div class="notif-time">${n.time}</div>
    </div>`).join('') || '<div style="color:var(--muted);padding:20px;">No notifications yet</div>';
}

function renderMessages() {
  document.getElementById('convo-items').innerHTML = DB.users.filter(u=>u.id!==currentUser.id).map(u=>{
    ...
    return `<div class="convo-item" id="convo-${u.id}" onclick="openChat('${u.id}')">
      <div class="avatar sm">${u.emoji}</div>
      <div style="flex:1;min-width:0;"><div class="name">${u.name}</div><div class="preview">${last?last.text:'Say hi!'}</div></div>
    </div>`;
  }).join('');
}
...
function openChat(userId) {
  ...
  document.getElementById('chat-area').innerHTML = `
    <div class="chat-header"><div class="avatar sm">${u.emoji}</div><div><div style="font-weight:600;">${u.name}</div><div style="font-size:12px;color:var(--muted);">@${u.username}</div></div></div>
    <div class="chat-messages" id="chat-messages">${msgs.map(m=>`<div class="msg-bubble ${m.from===currentUser.id?'mine':''}"><div class="bubble-inner">${m.text}</div><div class="bubble-time">${m.time}</div></div>`).join('')}</div>
    <div class="chat-input-area"><input class="chat-input" id="chat-input" placeholder="Message..." onkeydown="if(event.key==='Enter')sendMessage()" /><button class="send-btn" onclick="sendMessage()">➤</button></div>`;
...
function sendMessage() {
  ...
  const partnerId = activeConvoId;
  setTimeout(()=>{
    if (activeConvoId!==partnerId) return;
    ...
    DB.messages[key].push({ from:partnerId, text:reply, time:t2 });
    ...
    if (cm2) {
      ...
      cm2.appendChild(div); cm2.scrollTop=cm2.scrollHeight;
    }
    renderMessages();
  }, 1500);
  renderMessages();
...
function renderProfile() {
  ...
  document.getElementById('profile-posts-grid').innerHTML = myPosts.length
    ? myPosts.map(p=>`<div class="profile-post-tile">${p.emoji||'📝'}<div style="position:absolute;bottom:0;left:0;right:0;padding:6px;background:linear-gradient(0deg,rgba(0,0,0,.6),transparent);font-size:11px;">❤️${p.likes.length}</div></div>`).join('')
    : '<div style="color:var(--muted);padding:20px;font-size:14px;">No posts yet. Share something!</div>';
}
